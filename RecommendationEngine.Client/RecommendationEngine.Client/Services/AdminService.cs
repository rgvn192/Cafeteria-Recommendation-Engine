...
namespace RecommendationEngine.Client.Services
{
    ...
    public class AdminService : IAdminService
    {
        ...
        public async Task ShowMenuForThirdPartyServerAsync(NetworkStream stream, string role) // Include the role parameter
        {
            while (true)
            {
                Console.WriteLine("Admin Menu:");
                Console.WriteLine("1. Show Menu Item");
                ...
                {
                    case "1":
                        await ShowMenuItemsForThirdPartyServer(stream, role); // Pass the role parameter
                        break;
                    case "2":
                        Environment.Exit(0);
                        break;
        ...
        private async Task ShowMenuItemsForThirdPartyServer(NetworkStream stream, string role) // Include the role parameter
        {

            var getMenuItemsRequestModel = new GetMenuItemRequest()
            {
                endpoint = "/view-menu"
            };

            var response = await SendRequestToThirdPartyServerAsync<GetMenuItemRequest, ViewMenuResponseOfThirdParty>(stream, getMenuItemsRequestModel, _logger);

            if (response != null && response.status.ToLower() == "success")
            {
                var getMenuItemResponse = response.menu;
                if (getMenuItemResponse != null)
                {
                    foreach (var item in getMenuItemResponse)
                    {
                        Console.WriteLine($"{item.ID}\t{item.Name}\t{item.Price}\t{item.AvailabilityStatus}\n");
                    }
                }
            }
        }
        ...
        private async Task<TResponse> SendRequestToThirdPartyServerAsync<TRequest, TResponse>(NetworkStream stream, TRequest request, ILogger logger)
        {
            var requestData = Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(request));
            await stream.WriteAsync(requestData, 0, requestData.Length);
