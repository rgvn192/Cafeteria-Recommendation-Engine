    ...
    public class CommandHandlerRegistry : ICommandHandlerRegistry
    {
        ...
        public CommandHandlerRegistry(IServiceProvider serviceProvider)
        {
            CommandHandlers = new Dictionary<string, (CommandHandler, AuthorizationCheck)>
            {
                ...
                { "AddUserPreference", (body => AddUserPreference(serviceProvider, body), role => role == Roles.User.ToString()) },
                { "DeleteUserPreference", (body => DeleteUserPreference(serviceProvider, body), role => role == Roles.User.ToString()) },
                { "GetUserPreferences", (body => GetUserPreferences(serviceProvider, body), role => role == Roles.User.ToString()) },
                { "GetCharacteristics", (body => GetCharacteristics(serviceProvider, body), role => role == Roles.User.ToString()) },
